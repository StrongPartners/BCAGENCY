...
const Contact = () => {
    ...
    const channels = [
        { icon: <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>, label: isTr ? "WhatsApp'ta yazisalim" : "Let's chat on WhatsApp", value: '+90 548 832 19 19', href: 'https://wa.me/905488321919', primary: true },
        { icon: <svg viewBox="0 0 24 24" className="w-6 h-6 fill-none stroke-current stroke-2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.12.8.36 1.58.62 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l2.27-2.27a2 2 0 012.11-.45c1.23.26 2.01.5 2.81.62A2 2 0 0122 16.92z" /></svg>, label: isTr ? 'Telefon' : 'Call us', value: '+90 548 832 19 19', href: 'tel:+905488321919' },
        { icon: <svg viewBox="0 0 24 24" className="w-6 h-6 fill-none stroke-current stroke-2"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>, label: isTr ? 'E-posta' : 'Email us', value: 'info@bccreative.agency', href: 'mailto:info@bccreative.agency' },
        { icon: <Instagram className="w-6 h-6" />, label: 'Instagram', value: '@bccreative.agency', href: 'https://www.instagram.com/bccreative.agency/' },
    ];
                            ...
                            {status === 'success' ? (
                                ...
                                <form onSubmit={handleSubmit} className="space-y-5">
                                    ...
                                    <button type="submit" disabled={status === 'sending'} className="w-full flex items-center justify-center gap-2 bg-white text-ink-900 font-medium py-4 rounded-full hover:bg-secondary-300 transition-colors disabled:opacity-60">
                                        {status === 'sending' ? <><Loader2 size={18} className="animate-spin" />{isTr ? 'Gonderiliyor...' : 'Sending...'}</> : <><Send size={18} />{isTr ? 'Gonder' : 'Send message'}</>}
                                    </button>
                                </form>
                            )}
                        </ScrollText>
                            ...
                            {channels.map((ch, i) => (
                                <ScrollText key={ch.label} delay={i * 0.08}>
                                    <a href={ch.href} target={ch.href.startsWith('http') ? '_blank' : undefined} rel={ch.href.startsWith('http') ? 'noopener noreferrer' : undefined}
                                        ...
                                        <div className="w-11 h-11 bg-white/10 rounded-xl flex items-center justify-center shrink-0">{ch.icon}</div>
                            ...
                            <div className="space-y-5">
                                <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                                    <div className="flex items-start gap-4">
                                        <MapPin size={20} className="text-white/40 shrink-0 mt-1" />
                                        <div>
                                ...
                                <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                                    <div className="flex items-start gap-4">
                                        <Clock size={20} className="text-white/40 shrink-0 mt-1" />
                                        <div>
